...
struct LogHandlerPrivate {
  ...
  // 打开日志文件
  // log.txt，如果日志文件不是当天创建的，则使用创建日期把其重命名为
  // yyyyMMdd.log，并重新创建一个 log.txt
  ...
  static QTextStream
      *logOut;            // 输出日志的 QTextStream，使用静态对象就是为了减少函数调用的开销
...
QString LogHandlerPrivate::logfileName = "EmptyBoxClient";
...
LogHandlerPrivate::LogHandlerPrivate() {
  ...
  // 日志文件创建的时间
  // QFileInfo::created(): On most Unix systems, this function returns the time
  // of the last status change.
  // 所以不能运行时使用这个函数检查创建时间，因为会在运行时变化，所以在程序启动时保存下日志文件创建的时间
  // 在 Windows 又遇到一 Bug，log.txt
  // 的创建时间总是过去的某一天，就算新创建的也不会变，其实这里使用
  ...
  // renameLogFileTimer.setInterval(1000); //
  // 为了快速测试看到日期变化后是否新创建了对应的日志文件，所以 1 秒检查一次
...
void LogHandlerPrivate::openAndBackupLogFile() {
  // 总体逻辑:
  // 1. 程序启动时 logFile 为 NULL，初始化 logFile，有可能是同一天打开已经存在的
  // logFile，所以使用 Append 模式
  // 2. logFileCreatedDate is null,
  // 说明日志文件在程序开始时不存在，所以记录下创建时间
  // 3. 程序运行时检查如果 logFile
  // 的创建日期和当前日期不相等，则使用它的创建日期重命名，然后再生成一个新的
  // log.txt 文件
  ...
  if (nullptr == logFile) {
    ...
    logOut = (logFile->open(QIODevice::WriteOnly | QIODevice::Text |
                            QIODevice::Append))
  ...
  // [[3]]
  // 程序运行时如果创建日期不是当前日期，则使用创建日期重命名，并生成一个新的
  // log.txt
  if (logFileCreatedDate != QDate::currentDate()) {
    ...
    QString newLogPath = logDir.absoluteFilePath(
        logfileName + logFileCreatedDate.toString("yyyyMMdd") + ".log");
    QFile::copy(logPath, newLogPath); // Bug: 按理说 rename 会更合适，但是
                                      // rename
    // 时最后一个文件总是显示不出来，需要
    // killall Finder 后才出现
...
void LogHandler::installMessageHandler() {
  ...
  if (nullptr == d) {
    ...
    qInstallMessageHandler(
        LogHandlerPrivate::messageHandler); // 给 Qt 安装自定义消息处理函数
  }
}
